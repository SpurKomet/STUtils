package de.spurowski.stutils.util;
...
public class Reset {
    public void doReset(){
        if (!JMain.getInstance().getConfig().contains("isReset")){
            JMain.getInstance().getConfig().set("isReset", false);
            JMain.getInstance().saveConfig();
            return;
        }
        if (JMain.getInstance().getConfig().getBoolean("isReset")){
            try {
